...
it("should allow deep", () => {
  ...
  const A = Dict({ type: "A", value: Optional(Big) });
  const B = Dict({ type: "B", value: Optional(Multiple(Big)) });
  ...
  const D = Dict({ type: "D", value: C });
  const E = Dict({ type: "E", value: Optional(C) });

  const F = Multiple(
    DiscriminatedUnion([
      D,
      E,
      { type: "F" as const, value: Optional(C) },
      { type: "Fe" as const, value: Optional(C) },
      { type: "Fu" as const, value: Optional(C) },
      { type: "Fa" as const, value: Optional(C) },
      { type: "Fk" as const, value: Optional(C) },
      { type: "Fj" as const, value: Optional(C) },
      { type: "Fq" as const, value: Optional(C) },
    ]),
  );

  class TEST extends Dict({ value: F }) {}

  // class Test extends Shape({ test: F }) {
  //   deep = true as const;
  // }

  // Test.deserialize({});

  // const a = new Test(new A({ type: "A" }));
  // ex(a.deep).toBe(true as const).ok;
});
