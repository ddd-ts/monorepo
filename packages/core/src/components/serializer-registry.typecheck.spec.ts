...
async function concrete() {
  ...
  function requiresA(x: ISerializer<A>) {}

  // @ts-expect-error
  requiresA(new SerializerRegistry());
  // @ts-expect-error
  requiresA(new SerializerRegistry().auto(B));

  requiresA(new SerializerRegistry().auto(A));

  function requiresAandB(x: ISerializer<A> & ISerializer<B>) {}

  // @ts-expect-error
  requiresAandB(new SerializerRegistry());

  // @ts-expect-error
  requiresAandB(new SerializerRegistry().auto(A));

  // @ts-expect-error
  requiresAandB(new SerializerRegistry().auto(B));

  requiresAandB(new SerializerRegistry().auto(A).auto(B));
}
...
async function mergeChain() {
  ...
  class ASerializer extends AutoSerializer(A, 1) {}
  const aSerializerRegistry = new SerializerRegistry().add(
    A,
    new ASerializer(),
  );
  ...
  class BSerializer extends AutoSerializer(B, 1) {}
  const bSerializerRegistry = new SerializerRegistry().add(
    B,
    new BSerializer(),
  );
  ...
  const cSerializerRegistry = new SerializerRegistry().add(
    C,
    new CSerializer(),
  );
  ...
  const dSerializerRegistry = new SerializerRegistry().add(
    D,
    new DSerializer(),
  );

  const a = new SerializerRegistry()
    .merge(aSerializerRegistry)
    .merge(bSerializerRegistry)
    .merge(cSerializerRegistry)
    .merge(dSerializerRegistry);
