...
import { MakeEventStreamAggregateStore } from "./event-stream.aggregate-store";
import {
...
import { EventsOf } from "../traits/event-sourced";
...
export function EventStreamAggregateStoreSuite(config: {
  ...
  const accountStore = new AccountStore(
    accountEventStream,
    config.transaction,
    config.getSnapshotter("Account", registry.get(Account)),
  );
